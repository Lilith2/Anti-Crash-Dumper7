...
UEClass UEObject::GetClass() const
{
	if (Object == nullptr || Object == 0)
		return UEClass(NULL);
	char zeroBuffer[sizeof(UEClass(*reinterpret_cast<void**>(Object + Off::UObject::Class)))] = {}; // Create a zero-initialized buffer.
	auto temp = UEClass(*reinterpret_cast<void**>(Object + Off::UObject::Class));
	if (memcmp(&temp, zeroBuffer, sizeof(temp) == 0)) {
		// Entire object is zeroed out, invalid memory.
		return UEClass(NULL);
	}
...
{
	UEClass c = GetClass();
	char zeroBuffer[sizeof(c)] = {}; // Create a zero-initialized buffer.
	if (memcmp(&c, zeroBuffer, sizeof(c)) == 0) {
		// Entire object is zeroed out, invalid memory.
		return false;
	}
	return c.IsType(TypeFlags);
}
